...
const UP_NEXT = [
  { name: 'David Kim', detail: '#08 · Urgent · Waiting 3 min', urgent: true },
  { name: 'Ahmed Nasser', detail: '#06 · Waiting 22 min', urgent: false },
  { name: 'Emily Zhou', detail: '#07 · Waiting 15 min', urgent: false },
];

function ConsultationPanel({ onOpenUpNext }: { onOpenUpNext?: () => void }) {
  return (
    <>
        ...
        <div className="flex items-center gap-md">
          ...
          <div className="flex flex-col gap-[2px]">
            <p className="text-heading-sm text-text-primary">
              {ACTIVE_PATIENT.name} · #{ACTIVE_PATIENT.queueNumber}
            </p>
            <p className="text-body-sm text-text-secondary">{ACTIVE_PATIENT.details}</p>
          </div>
        </div>
        <p className="text-label-sm text-text-tertiary">ACTIVE THERAPIES</p>
        {ACTIVE_PATIENT.activeTherapies.map((therapy) => (
          <p key={therapy} className="text-body-md text-text-primary">
            ☐ {therapy}
          </p>
        ))}
        <p className="text-label-sm text-text-tertiary">VISIT HISTORY</p>
        <p className="text-body-sm text-text-secondary">{ACTIVE_PATIENT.visitHistory}</p>
      </div>
...
function UpNextRows() {
  return (
    <>
      {UP_NEXT.map((patient) => (
        <div key={patient.name} className="flex items-start gap-[12px] rounded-md bg-bg-surfaceAlt px-[14px] py-sm">
          <div
...
function UpNextDrawer({ open, onClose }: { open: boolean; onClose: () => void }) {
  return (
    <>
      <div
        onClick={onClose}
      ...
      >
        <div className="flex flex-col gap-md">
          ...
          <Button variant="primary">Call Next Patient</Button>
          <UpNextRows />
...
export function DoctorWaitingRoom() {
  ...
  return (
    <div className="flex flex-col gap-lg">
      <ScreenHeader role="doctor" title="Room 3 · Consultation" subtitle="5 patients waiting" />
      <ConsultationPanel onOpenUpNext={() => setIsUpNextOpen(true)} />
      <UpNextDrawer open={isUpNextOpen} onClose={() => setIsUpNextOpen(false)} />
    </div>
  );
}
